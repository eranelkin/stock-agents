...
- **Volume $ →** `volume_dollar` (pre-market volume × pre-market price, pre-calculated): consume directly — do not recompute. Represents total pre-market dollar flow for this ticker. Output the value as-is in the `volume_dollar` field; output null if not provided.
- **Ratio Vol / Market Cap →** `ratio_vol_market_cap` (volume_dollar ÷ market_cap, pre-calculated): consume directly — do not recompute. Values >0.10 indicate significant pre-market dollar flow relative to the company's market cap. Use as a supporting signal in Volume & Microstructure (Pillar B) scoring. Output the value as-is in the `ratio_vol_market_cap` field; output null if not provided.
