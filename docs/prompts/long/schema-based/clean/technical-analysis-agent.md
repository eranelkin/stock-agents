...
Analyze the prompt and extract all the input required for this prompt to run
...
The input inline json is your absolute ground truth for the specific variables it contains. You must lock in the provided symbol, price, pre_market_price, pre_parket_volume, market_cap, and atr. Do not attempt to verify, update, or fetch these specific fields from external sources.
...
CRITICAL - YOU HAVE REPEATABLES MISTAKES WITH ACCURACY - VERIFY PARAMS FROM THE JSON TAKEN ACCURATELY!!
