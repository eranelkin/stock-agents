...
/** Fetch the global ticker input schema from backend/stock_schema.json. */
...
export async function fetchActiveAgentPrompts(): Promise<Prompt[]> {
  ...
  if (!res.ok)
    throw new Error(`Failed to fetch agent prompts: ${res.statusText}`);
...
export async function createPrompt(payload: {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      body.detail ?? `Failed to create prompt: ${res.statusText}`,
    );
  }
  return res.json();
}
...
export async function updatePrompt(
  ...
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
    throw new Error(
      body.detail ?? `Failed to update prompt: ${res.statusText}`,
    );
  }
  return res.json();
}
